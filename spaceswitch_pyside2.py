...
class UI(QtWidgets.QDialog):
    ...
    def create_widgets(self):
        ...
        self.separator_target = QtWidgets.QFrame(
    ...
    def showEvent(self, event):
        self.idx = om.MEventMessage.addEventCallback('SelectionChanged',
                                                     self.refresh)
    ...
    def refresh(self, *args):
        ...
        if len(sel) > 0:
            if self.namespaces:
                ...
                self.selection.setAlignment(QtCore.Qt.AlignLeft
                                            | QtCore.Qt.AlignVCenter)
            else:
                self.selection.setText(sel[0])
                self.selection.setAlignment(QtCore.Qt.AlignRight
                                            | QtCore.Qt.AlignVCenter)
            self.attribute_btn.hide()
            ...
            if len(sel) == 1:
                ...
                self.selected_target.setAlignment(QtCore.Qt.AlignLeft
                                                  | QtCore.Qt.AlignVCenter)
            else:
                self.show_hide_target()
                if not self.namespaces:
                    self.selected_target.setAlignment(QtCore.Qt.AlignRight
                                                      | QtCore.Qt.AlignVCenter)
                else:
                    self.selected_target.setAlignment(QtCore.Qt.AlignLeft
                                                      | QtCore.Qt.AlignVCenter)
    ...
    def getEnum(self):
        ...
        if allAttrs and cbAttrs:
            orderedAttrs = [
                attr for attr in allAttrs for cb in cbAttrs
                if cb.endswith(attr)
            ]
            for i in orderedAttrs:
                ...
                if attrType == 'enum':
                    enum_values = cmds.attributeQuery(
                        i, node=sel, listEnum=True)[0].split(':')
    ...
    def select_attr(self):
        sel = self.getSelectedObj()[0]
        selected_channelbox = cmds.channelBox('mainChannelBox',
                                              q=True,
                                              sma=True)
        attrType = cmds.attributeQuery(selected_channelbox,
                                       node=sel,
                                       attributeType=True)
        if selected_channelbox:
            if not attrType == 'enum':
                cmds.warning(
                    'Ensure that the selected attribute is an enum type.')
                return
    ...
    def set_combobox(self, sel, enum_attr):
        ...
        enumOptions = cmds.attributeQuery(enum_attr, node=sel,
    ...
    def show_hide_target(self, button_triggered=False):
        ...
        for widget in (self.target_layout.itemAt(i)
    ...
    def apply_changes(self):
        ...
        def multiple_frames(keyframes):
            ...
            try:
                ...
                for frame in keyframes:
                    ...
                    list_xform.append(
                        cmds.xform(target, q=True, ws=True, matrix=True))
                    cmds.progressBar(gMainProgressBar,
                                     edit=True,
        ...
        if keyframes:
            if cmds.timeControl('timeControl1', rv=1, q=True):
                keyframes = sorted(frame for frame in keyframes
                                   if timeline[0] <= frame <= timeline[1])
                if len(keyframes) < 2:
